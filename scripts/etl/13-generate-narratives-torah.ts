// 13-generate-narratives-torah.ts — Insert pre-baked Layer 2D narrative units for Torah
import { db, heading, log } from './db'
import type { NarrativeUnitRecord } from './data/narrative-types'

import { narrativesGenesis1 } from './data/narratives-genesis-1'
import { narrativesGenesis2 } from './data/narratives-genesis-2'
import { narrativesExodus } from './data/narratives-exodus'
import { narrativesLeviticus } from './data/narratives-leviticus'
import { narrativesNumbers } from './data/narratives-numbers'
import { narrativesDeuteronomy } from './data/narratives-deuteronomy'
...
const ALL_NARRATIVES: NarrativeUnitRecord[] = [
  ...
  ...narrativesLeviticus,
  ...narrativesNumbers,
  ...narrativesDeuteronomy,
]

function main(): void {
  heading('Importing Layer 2D — Narrative Units (Torah)')
  ...
  // Summary by book
  const byBook = db.prepare('SELECT book_id, COUNT(*) as c FROM narrative_units GROUP BY book_id ORDER BY MIN(chapter_start)').all() as { book_id: string; c: number }[]
