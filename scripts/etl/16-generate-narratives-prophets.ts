// 16-generate-narratives-prophets.ts — Insert pre-baked Layer 2D narrative units for Prophetic Books
import { db, heading, log } from './db'
import type { NarrativeUnitRecord } from './data/narrative-types'

import { narrativesMajorProphets } from './data/narratives-major-prophets'
import { narrativesDaniel } from './data/narratives-daniel'
import { narrativesMinorProphetsA } from './data/narratives-minor-prophets-a'
import { narrativesMinorProphetsB } from './data/narratives-minor-prophets-b'

const ALL_NARRATIVES: NarrativeUnitRecord[] = [
  ...narrativesMajorProphets,
  ...narrativesDaniel,
  ...narrativesMinorProphetsA,
  ...narrativesMinorProphetsB,
]

function main(): void {
  heading('Importing Layer 2D — Narrative Units (Prophets)')
  ...
  const byBook = db.prepare(`SELECT book_id, COUNT(*) as c FROM narrative_units
    WHERE book_id IN ('ISA','JER','LAM','EZK','DAN','HOS','JOL','AMO','OBA','JON','MIC','NAM','HAB','ZEP','HAG','ZEC','MAL')
