...
import { Plus, Presentation, ChevronRight } from 'lucide-react'
...
export function ProjectLanding({ projects, onCreateProject, onOpenProject }: StudyBuilderProps) {
  ...
  return (
    <div className="h-full overflow-y-auto" style={{ padding: '40px 32px' }}>
      <div style={{ maxWidth: '640px', margin: '0 auto' }}>
        ...
        {projects.length > 0 ? (
          <div className="space-y-3">{projects.map((project) => (<ProjectCard key={project.id} project={project} onOpen={() => onOpenProject?.(project.id)} />))}</div>
        ) : (
